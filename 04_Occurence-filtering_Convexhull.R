...
Rastab <- as.data.frame(baseline_raster, xy = TRUE, na.rm = FALSE) 
...
for (i in 1:length(Vect_Sp)) {
  ...
  if (length(Occu$x) > 50) {
    ...
    Occu_r <- as.data.frame(Occu_r, xy = TRUE, na.rm = FALSE) # Convert to data frame with coordinates
    ...
    # Final dataframe of occurrences and their respective variable values
    ...
    xlsx::write.xlsx(Fin_occ_var, paste0("data/filtered_occurences/Occ&Var_final", Sp, ".xlsx"), row.names = FALSE)
    ...
    }, int.to.replace = env.itvl_2, replacing.values = envir.space$detailed.intervals)
    ...
    outs <- lapply(colnames(var.occ_2), function(x, df) {
      qt <- quantile(df[, x], probs = c(0 + convex.hull.interval / 2, 1 - convex.hull.interval / 2))
    ...
    cursp.inhull <- geometry::inhulln(cursp.convhull, as.matrix(envir.space$unique.conditions.in.env))
    ...
    # Check where species occurrences are located into the environmental space (using midpoints variable values)
