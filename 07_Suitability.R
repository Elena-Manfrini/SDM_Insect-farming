...
for (i in 1:7) {
 ...
 ### 2.1 Projection Mean : Ensemble model
  ...
  # B. Standard deviation map
    ...
    saveRDS(raster_sd_df, paste0("output/suitability/", Sp, "_sd_ens_mod.RDS"))
  ...
  labs(title = "Standard deviation Raster",
    ...
    # Save the plot
      ...
      ## 2.2 Risk classification map visualization
  ...
  units = "in"
    ...
    ########################## 3. Highest categories
    # Collect all High suitability of species
...
##### 3.1 Plot of cumulated High risk for all species
